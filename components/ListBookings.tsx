...
export default function ListBookings() {
  ...
  const [vehiclesFilter, setVehiclesFilter] = useState<number[]>(
    vehicleOptions.map((vehicle) => vehicle.id)
  );
  ...
  function onChangeVehiclesFilter(vehicleId: number) {
    if (vehiclesFilter.includes(vehicleId)) {
      setVehiclesFilter(
        vehiclesFilter.filter((vehicleFilter) => vehicleFilter != vehicleId)
      );
    } else {
  ...
  async function getBookings() {
    ...
    if (pickupDateFilter != "") {
      filtersCount++;
      query.gt("pickup", pickupDateFilter);
    }
    if (dropoffDateFilter != "") {
      filtersCount++;
      query.lt("dropoff", dropoffDateFilter);
    }
    ...
    <div className="flex flex-col p-4 h-fit w-full rounded-lg bg-white shadow-md max-w-sm">
      ...
      {!isLoading && (
        <button
          ...
          ${isFilterOn ? "bg-gray-100 shadow-inner" : "bg-white"}
        `}
        >
          {filtersOn > 0 && (
        ...
        <div className="flex flex-col gap-4 mb-4 border border-gray-100 rounded-lg p-2">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <DatePicker
                id="pickup-start"
                selectedDate={pickupDateFilter}
                onChange={(newDate) => setPickupDateFilter(newDate)}
                showPastDates
              />
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <DatePicker
                id="dropoff-start"
                selectedDate={dropoffDateFilter}
                onChange={(newDate) => setDropoffDateFilter(newDate)}
                showPastDates
              />
            </div>
          </div>
      ...
      {isLoading ? (
        <ArrowPathIcon
          className={"animate-spin text-gray-200 mx-auto my-4 h-12 w-12"}
        />
      ) : (
        bookings &&
        bookings.length > 0 &&
        bookings.map((booking, index) => (
          <div
            key={index}
            className={`w-full flex flex-col max-w-sm mt-4 gap-4 p-4 rounded-md border 
            ${
              booking.active
          ...
          >
            <div className="flex flex-row gap-4">
              <div className="flex items-center bg-gray-100 -mt-4 -ml-4 rounded-md shadow p-2 sm:p-4">
                <img className={`w-fit h-10 sm:h-16`} src={booking.car.image} />
              </div>
            ...
            {booking.active ? (
              ...
              <p className="text-sm ml-auto font-semibold">Canceled</p>
            )}
          </div>
        ))
      )}
