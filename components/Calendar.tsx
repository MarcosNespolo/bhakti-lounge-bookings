...
export default function Calendar({ id, selectedDate, unavailableDates, onAction }: CalendarType) {
    ...
    const freeTimes = useMemo(() => {
        ...
        if (unavailableDates) {
            unavailableDates.forEach(unavailableDate => {
                freeTimes = freeTimes.filter(hour =>
        ...
        <div className="flex flex-col w-full justify-center items-center gap-2 bg-gray-50 rounded-lg">
            ...
            <div className="flex flex-col h-fit w-full p-2">
                ...
                <div className="flex flex-row w-full items-center text-gray-600">
                    <button
                        type="button"
                        className={`flex justify-end rounded-md p-1 ${!isThisMonth(new Date(currentMonth)) && 'hover:bg-secondary'}`}
                        onClick={prevMonth}
            ...
            <div className={cn(`hidden`, calendarTouched && "block")}>
                <AvailableHours freeTimes={freeTimes} confirmOption={(selectedTime) => onAction(selectedTime)} isPickup />
            </div>
        </div>
    )
}
